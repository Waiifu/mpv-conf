...
function create_continuous_page_stitches(start, last)
	...
	for i=start,last do
		...
		pageOffset = pageOffset+opts.continuous_size
		local top_page = filearray[i+pageOffset]
		local bottom_page = filearray[i+opts.continuous_size-1+pageOffset]
		if not (filearray[start+pageOffset] and filearray[last+pageOffset] and filearray[i+opts.continuous_size+pageOffset]) then
			return
		end
		local name = generate_name(filearray[i+pageOffset], filearray[i+opts.continuous_size+pageOffset])
